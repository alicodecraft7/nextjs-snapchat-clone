  ...
  return (
    <div className="flex-1 my-3 border-2 border-gray-300 overflow-y-auto p-2 rounded-lg">
      {messages?.map((message: any, index: number) => {
        const isLoggedInUser = message.senderId._id == authUser?.user?._id;
            ...
            {!isPreviousMessageFromSameSender && (
              <p
                className={`font-bold mt-2 text-xs ${
                  isLoggedInUser ? "text-red-500" : "text-[#00b4d8]"
                } `}
              >
                {isLoggedInUser ? "Me" : senderFullName}
              </p>
            )}
            <div
              className={`border-l-2 ${
                isLoggedInUser ? "border-l-red-500" : "border-l-[#00b4d8]"
              }`}
            >
